...
from wnsm.api.constants import AnlagenType, RoleType, ValueType  # noqa: E402
...
zp_template = {
    ...
    "anlage": {"typ": "TAGSTROM"},
    "vertraege": [{"einzugsdatum": "2010-01-01", "auszugsdatum": "2015-12-31"}],
    "idexStatus": {
        "granularity": {"status": "QUARTER_HOUR", "canBeChanged": True},
        "customerInterface": {"status": "active", "canBeChanged": True},
        "display": {"isLocked": True, "canBeChanged": True},
        "displayProfile": {"canBeChanged": True, "displayProfile": "VERBRAUCH"},
    },
    "optOutDetails": {"isOptOut": False},
    "zpSharingInfo": {"isOwner": False},
}
...
def verbrauch_raw_response():
    return {
        ...
        "statistics": {"maximum": 6918, "minimum": 3856, "average": 5301},
    }
    ...
    for i in range(zaehlwerk_amount):
        if no_obis_code:
            ...
            if all_invalid_obis:
                ...
                all_valid_obis or i == 0
            ):  # First one valid, or all valid if multiple_valid_obis is True
                obis = valid_obis_codes[i % len(valid_obis_codes)]
            else:  # Default: first valid, rest invalid
                obis = invalid_obis_code
            zaehlwerke.append(
                {"obisCode": obis, "einheit": "WH", "messwerte": messwerte}
            )
...
def bewegungsdaten_response(
    customer_id: str,
    zp: str,
    granularity: ValueType = ValueType.QUARTER_HOUR,
    anlagetype: AnlagenType = AnlagenType.CONSUMING,
    wrong_zp: bool = False,
    values_count: int = 10,
):
    ...
    values = (
        ...
        if values_count == 0
            ...
            count=values_count, timestamp=datetime(2022, 8, 7, 0, 0, 0), interval=gran
        )
    )
...
def smartmeter(username=USERNAME, password=PASSWORD, code_verifier=CODE_VERIFIER):
    return api.client.Smartmeter(
        username=username, password=password, input_code_verifier=code_verifier
    )
...
def mock_get_api_key(
    ...
    bearer_token: str = ACCESS_TOKEN,
    get_config_status: int | None = 200,
    include_b2c_key: bool = True,
    include_b2b_key: bool = True,
    same_b2c_url: bool = True,
    same_b2b_url: bool = True,
):
    """
    ...
    if not same_b2c_url:
        config_data["b2cApiUrl"] = (
            "https://api.wstw.at/gateway/WN_SMART_METER_PORTAL_API_B2C/2.0"
        )
    if not same_b2b_url:
        config_data["b2bApiUrl"] = (
            "https://api.wstw.at/gateway/WN_SMART_METER_PORTAL_API_B2B/2.0"
        )
    ...
    if get_config_status is None:
        ...
        requests_mock.get(
            url=API_CONFIG_URL,
            request_headers={"Authorization": f"Bearer {bearer_token}"},
            status_code=get_config_status,
            text=config_response,
        )
...
def mock_token(
    ...
    if status == 200:
        requests_mock.post(
            f"{AUTH_URL}/token",
            additional_matcher=post_data_matcher(
                {
...
def mock_authenticate(
    ...
    if status == 302:
        redirect_url = (
            f"{REDIRECT_URI}/#state=cb142d1b-d8b4-4bf3-8a3e-92544790c5c4"
            "&session_state=949e0f0d-b447-4208-bfef-273d694dc633"
            f"&code={code}"
        )
        requests_mock.post(
        ...
        requests_mock.get(
            redirect_url, text="Some page which loads a js, performing a call to /token"
        )
        ...
        status == 404
    ):  # do provide a redirect to not-found page in Location header on 404 -> 302
        requests_mock.post(
            authenticate_url,
            status_code=302,
            additional_matcher=post_data_matcher(
        ...
        redirect_url = (
            f"{REDIRECT_URI}/#code={code}#state=cb142d1b-d8b4-4bf3-8a3e-92544790c5c4"
            "&session_state=949e0f0d-b447-4208-bfef-273d694dc633"
        )
        requests_mock.post(
            authenticate_url,
...
def expect_verbrauch(
    ...
    params = {"dateFrom": _dt_string(dateFrom), "dayViewResolution": resolution}
    path = f"messdaten/{customer_id}/{zp}/verbrauch?{urlencode(params)}"
    requests_mock.get(
        parse.urljoin(API_URL_B2C, path),
        headers={
            "Authorization": f"Bearer {ACCESS_TOKEN}",
            "X-Gateway-APIKey": B2C_API_KEY,
        },
...
def expect_history(
    ...
    requests_mock.get(
        parse.urljoin(API_URL_B2B, path),
        headers={
            "Authorization": f"Bearer {ACCESS_TOKEN}",
...
def expect_bewegungsdaten(
    ...
    url = parse.urljoin(
        API_URL_ALT, f"user/messwerte/bewegungsdaten?{urlencode(params)}"
    )
    requests_mock.get(
        url,
        headers={
        ...
        json=bewegungsdaten_response(
            customer_id, zp, granularity, anlagetype, wrong_zp, values_count
        ),
    )
