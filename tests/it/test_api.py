...
@pytest.mark.usefixtures("requests_mock")
def test_unsuccessful_login_failing_on_connection_timeout_while_login_page_load(
    requests_mock,
):
...
@pytest.mark.usefixtures("requests_mock")
def test_unsuccessful_login_failing_on_redirect_when_location_header_does_not_bear_code(
    requests_mock,
):
    mock_login_page(requests_mock)
    ...
    with pytest.raises(SmartmeterLoginError) as exc_info:
        smartmeter().login()
    assert "Login failed. Could not extract 'code' from 'Location'" in str(
        exc_info.value
    )


@pytest.mark.usefixtures("requests_mock")
def test_unsuccessful_login_failing_on_connection_timeout_while_retrieving_access_token(
    requests_mock,
):
    mock_login_page(requests_mock)
    mock_authenticate(requests_mock, USERNAME, PASSWORD)
...
@pytest.mark.usefixtures("requests_mock")
def test_unsuccessful_login_failing_on_empty_response_while_retrieving_access_token(
    requests_mock,
):
    mock_login_page(requests_mock)
    mock_authenticate(requests_mock, USERNAME, PASSWORD)
...
@pytest.mark.usefixtures("requests_mock")
def test_unsuccessful_login_failing_on_connection_timeout_while_get_config_on_get_api_key(
    requests_mock,
):
    mock_login_page(requests_mock)
    mock_authenticate(requests_mock, USERNAME, PASSWORD)
...
def test_warning_b2c_api_key_change(requests_mock, caplog):
    ...
    smartmeter().login()
    assert (
        const.API_URL == "https://api.wstw.at/gateway/WN_SMART_METER_PORTAL_API_B2C/2.0"
    )
...
def test_history_invalid_obis_code(requests_mock: Mocker):
    ...
    with pytest.raises(SmartmeterQueryError) as exc_info:
        smartmeter().login().historical_data()
    assert "No valid OBIS code found. OBIS codes in data: ['9-9:9.9.9']" == str(
        exc_info.value
    )
...
def test_history_multiple_zaehlwerke_all_valid(requests_mock: Mocker, caplog):
    ...
    expect_login(requests_mock)
    expect_history(
        requests_mock, customer_id, zp, zaehlwerk_amount=3, all_valid_obis=True
    )
    expect_zaehlpunkte(requests_mock, [enabled(zaehlpunkt())])
    hist = smartmeter().login().historical_data()
    assert 1 == len(hist["messwerte"])
    ...
    assert "1-1:1.8.0" == hist["obisCode"]
    assert (
        "Multiple valid OBIS codes found: ['1-1:1.8.0', '1-1:1.9.0', '1-1:2.8.0']. Using the first one."
        in caplog.text
    )
...
def test_history_multiple_zaehlwerke_all_invalid(requests_mock: Mocker, caplog):
    ...
    expect_login(requests_mock)
    expect_history(
        requests_mock, customer_id, zp, zaehlwerk_amount=3, all_invalid_obis=True
    )
    expect_zaehlpunkte(requests_mock, [enabled(zaehlpunkt())])
    with pytest.raises(SmartmeterQueryError) as exc_info:
        smartmeter().login().historical_data()
    ...
    assert (
        "No valid OBIS code found. OBIS codes in data: ['9-9:9.9.9', '9-9:9.9.9', '9-9:9.9.9']"
        == str(exc_info.value)
    )
...
def test_history_no_zaehlwerke(requests_mock: Mocker, caplog):
    ...
    with pytest.raises(SmartmeterQueryError) as exc_info:
        smartmeter().login().historical_data()
    assert "Returned data: " in caplog.text
    assert "Returned data does not contain any zaehlwerke or is empty." == str(
        exc_info.value
    )
...
def test_bewegungsdaten_daily_consuming(requests_mock: Mocker):
    ...
    hist = (
        smartmeter().login().bewegungsdaten(None, dateFrom, dateTo, const.ValueType.DAY)
    )
...
def test_bewegungsdaten_no_dates_given(requests_mock: Mocker):
    ...
    expect_login(requests_mock)
    expect_bewegungsdaten(
        requests_mock, z["geschaeftspartner"], zpn, dateFrom, dateTo, values_count=COUNT
    )
    expect_zaehlpunkte(requests_mock, [enabled(zaehlpunkt())])
...
def test_verbrauch_raw(requests_mock: Mocker):
    ...
    expect_login(requests_mock)
    expect_history(
        requests_mock, customer_id, enabled(zaehlpunkt())["zaehlpunktnummer"]
    )
    expect_zaehlpunkte(requests_mock, [enabled(zaehlpunkt())])
    expect_verbrauch(
        requests_mock, customer_id, zp, dateFrom, valid_verbrauch_raw_response
    )
